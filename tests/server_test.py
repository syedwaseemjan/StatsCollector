...
from app.tasks import check_threshold, get_subject_text, \
					get_message_text, upload_collector
from app.models import get_or_create, save
...
class ServerTestCase(StatsCollectorAppTestCase):
	...
	def test_process_clients_file_incorrect_syntax(self):		
		clients_list = os.path.join(os.path.dirname(__file__), 'clients_incorrent.xml')
	...
	def test_check_threshold(self):
		...
		with patch("app.tasks.send_email.delay") as mock_task:
			data = parse_client(clients[0])
			stats.cpu_usage = 18
			stats.mem_usage = 55
